...
const StrengthBar = ({ score, visible }) => {
  ...
  return (
    <div style={{ marginTop: "0.5rem" }}>
      ...
      <div style={{ fontSize: "0.75rem", fontWeight: 700, color: level.color, letterSpacing: "0.02em" }}>
        {level.label}
      </div>
    </div>
  );
...
/* ── Requirement Item ─────────────────────────────────────────── */
const Req = ({ met, label }) => (
  <div style={{ display: "flex", alignItems: "center", gap: "7px", fontSize: "0.8rem", color: met ? "#16A34A" : "#6B7280", transition: "color 0.2s" }}>
    <div style={{
...
const EyeToggle = ({ show, onToggle }) => (
  <button type="button" className="auth-password-toggle" onClick={onToggle} tabIndex="-1" aria-label={show ? "Hide password" : "Show password"}>
    {show ? (
...
export const ResetPasswordPage = () => {
  ...
  const strength   = useMemo(() => zxcvbn(newPassword), [newPassword]);
  const score      = newPassword.length === 0 ? 0 : strength.score;
  const reqs       = useMemo(() => getRequirements(newPassword), [newPassword]);
  ...
  const confirmMismatch = touched && confirmPassword.length > 0 && newPassword !== confirmPassword;
          ...
          {!message && (
            <form onSubmit={handleSubmit} className="auth-form">
              ...
              <div className="auth-form-group">
                ...
                {/* Strength bar */}
                <StrengthBar score={score} visible={newPassword.length > 0} />

                {/* zxcvbn hint */}
                {newPassword.length > 0 && suggestions.length > 0 && score < 3 && (
                  <div style={{ marginTop: "6px", fontSize: "0.75rem", color: "#6B7280", fontStyle: "italic", lineHeight: 1.4 }}>
                    💡 {suggestions[0]}
                  </div>
                )}
              ...
              <div className="auth-form-group">
                ...
                {!confirmMismatch && confirmPassword.length > 0 && newPassword === confirmPassword && (
              ...
              <button type="submit" className="auth-submit-btn" disabled={submitting}>
                {submitting ? <div className="auth-spinner"></div> : "Reset Password"}
              </button>
            </form>
          )}

          {/* Link expired fallback */}
          {error && (
